...
export async function extractPeaks(file: File): Promise<number[] | null> {
  try {
    ...
    const peaks: number[] = new Array(SAMPLE_COUNT);

    // Merge all channels by taking the max amplitude per sample block
    for (let i = 0; i < SAMPLE_COUNT; i++) {
      const start = i * blockSize;
      const end = Math.min(start + blockSize, decoded.length);
      let max = 0;
